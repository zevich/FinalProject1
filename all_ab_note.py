...
class Email(Field):
    ...
    def value(self, value: str):
        if value is None:
            ...
            try:
                self.__value = re.findall(r'\b[a-zA-Z]{1}[\w\.]+@[a-zA-Z]+\.[a-zA-Z]{2,}', value)
            except ValueError:
...
class Record:
    def __init__(self, name: Name, phones=[], email=None, birthday=None) -> None:
        self.name = name
    ...
    def __str__(self) -> str:
        return f'User : {self.name.value} | Birthday : {self.birthday} | Email : {self.email} | {self.days_to_birthday(self.birthday)} day to birthday\n' \
...
def add_contact(contacts, *args):
    ...
    if name.value in contacts:
        ...
        if len(args) == 4:
            email = Email(args[2])
            #print (args[2])
            #result =  re.findall(r'\b[a-zA-Z]{1}[\w\.]+@[a-zA-Z]+\.[a-zA-Z]{2,}', args[2])
            #print (result[0])
            ...
            #print (result[0])
            #if (result[0] == args[2]):
            #if (re.findall(r'\b[a-zA-Z]{1}[\w\.]+@[a-zA-Z]+\.[a-zA-Z]{2,}', args[2]) == args[2]):   
            if  "@" in args[2]:   
                #print (f'&&&&&&&&11111')
                email = Email(args[2])
                ...
                #print(birthday)
                email = Email(None)
        contacts[name.value] = Record(name, [phone], email, birthday)
...
def search(contacts, *args):
    def func_sub(record):
        return substring.lower() in record.name.value.lower() or \
               ...
               (record.birthday.value is not None and substring in record.birthday.value.strftime('%d.%m.%Y')) or \
               (record.email.value is not None and substring.lower() in record.email.value[0].lower())
    substring = args[0]
...
def main():
    start_message = "### Console bot ###\n" \
                    "###  Team 7 - Troy  ###\n"
    print(start_message)
    time.sleep(1)
    start_message = "Choose mode:\n" \
                    "\t1   - Address book\n" \
                    "\t2   - Note\n" \
                    "\t3   - Esc - Exit\n" \
